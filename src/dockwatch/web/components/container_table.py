    ...
    def __init__(self) -> None:
        with ui.column().classes("w-full gap-2"):
            with ui.row().classes("w-full text-bold text-grey-7 hidden md:flex"):
                ...
                ui.label("Image").classes("w-2/12")
                ui.label("Deployed").classes("w-2/12")
                ui.label("Remote").classes("w-2/12")
                ui.label("Why").classes("w-1/12")
            ...
            for result in results:
                ...
                with ui.card().classes("w-full"):
                    with ui.row().classes("w-full items-center gap-2 flex-wrap md:flex-nowrap"):
                        ...
                        ui.label(result.container_info.image_ref or "-").classes("w-full md:w-2/12 break-all")
                        ui.label(deployed_display(result.container_info)).classes("w-full md:w-2/12")
                        ui.label(remote_value).classes("w-full md:w-2/12 break-all")
                        ui.label(reason_value).classes("w-full md:w-1/12 break-all text-grey-7")
